...
# Refer - video - https://www.youtube.com/watch?v=3Zv-s9UUrFM
    ...
    def spiralOrder(self, matrix: List[List[int]]) -> List[int]:
        ...
        while len(spiral_lst) < (len(matrix) * len(matrix[0])):
            ...
            # important to check the length of spiral_lst after each direction to avoid adding duplicate elements when there is only one row left in the matrix for testcase like matrix = [[1, 2, 3, 4]]
            ...
            # important to check the length of spiral_lst after each direction to avoid adding duplicate elements when there is only one column left in the matrix for testcase like matrix = [[1], [2], [3], [4]]
            if len(spiral_lst) == (len(matrix) * len(matrix[0])):
                break
